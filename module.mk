...
# CivetWeb
CFLAGS+=-DNO_SSL -DUSE_IPV6=1 -DUSE_WEBSOCKET=1
# date tz
...
CFLAGS+=-isystem$(TOPDIR)/foreign/civetweb/include
CFLAGS+=-isystem$(TOPDIR)/foreign/date/include
CFLAGS+=-isystem$(TOPDIR)/foreign/fmt/include
CFLAGS+=-isystem$(TOPDIR)/foreign/inja/include
CFLAGS+=-isystem$(TOPDIR)/foreign/libvlcpp
...
CFLAGS+=-isystem$(TOPDIR)/foreign/tartanllama_optional/include
CFLAGS+=-isystem$(TOPDIR)/foreign/CxxUrl
CFLAGS+=-isystem$(TOPDIR)/foreign/LURLParser
CFLAGS+=-isystem$(TOPDIR)/foreign/uri-library


EMBED:=listMedia.template create_database.sql footer.template header.template history.template playlist.template standby.png utuputki.css utuputki.js
...
EMBED_HEADERS:=$(foreach f, $(EMBED), $(f).h)
...
distclean: clean
	rm -f $(foreach dir,$(ALLDIRS),$(dir)/*.d)  $(EMBED_HEADERS)
...
#  $(call embed-file, filename, headername)
define embed-file
...
$(eval $(foreach f, $(EMBED), $(call embed-file, $(TOPDIR)/$(f), $(f).h ) ) )
